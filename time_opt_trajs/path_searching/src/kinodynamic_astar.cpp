...
namespace opt_planner
{
  ...
  int KinodynamicAstar::search(Eigen::MatrixXd startState, // 3 * 3
                               ...
                               bool init, bool dynamic)
  {
    ...
    while (!open_set_.empty())
    {
      ...
      //   display3DLine(cur_node->state.head(3), cur_node->parent->state.head(3), iter_num_);
      // }
      ...
      for (unsigned int j = 0; j < durations.size(); ++j)
      {
        ...
        for (unsigned int i = 0; i < inputs.size(); ++i)
        {
          ...
          //std::cout << "[KinodynamicAstar::search]:  pro_state " <<   pro_state  << std::endl;
  ...
  template <typename T>
  void KinodynamicAstar::retrievePath(T end_node,
                                      std::vector<T> &nodes)
  {
    T cur_node = end_node;
    nodes.clear();
  ...
  void KinodynamicAstar::intialMap(GridMap::Ptr &grid_map)
  {
  ...
  void KinodynamicAstar::init(int ego_id, double ego_radius)
  {
  ...
  void KinodynamicAstar::reset()
  {
    ...
    std::priority_queue<PathNodePtr, std::vector<PathNodePtr>, NodeComparator<PathNodePtr>> empty_queue;
  ...
  void KinodynamicAstar::getKinoTraj(double delta_t,
