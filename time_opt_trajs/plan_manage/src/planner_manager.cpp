...
void PlannerManager::initPlanModules(
    ros::NodeHandle &nh, const std::shared_ptr<MPL::VoxelMapUtil> &map_util,
    std::string frame_id) {
  ...
  poly_pub_ = nh.advertise<decomp_ros_msgs::PolyhedronArray>("sikangpolyhedron",
                                                             1, true);
}
...
bool PlannerManager::localPlanner(Eigen::MatrixXd &startState,
  ...
  std::vector<Eigen::Vector3d> path_pts = search_path;

  ros::Time time_now = ros::Time::now();
  auto start_timer = std::chrono::high_resolution_clock::now();

  // step two: kinodynamic path searching considering obstacles avoidance
  ROS_WARN("%d", search_path.size());
  if (path_pts.empty()) {
    ROS_WARN("Search path not given, running kinodynamic search");
    if (use_jerk_) {
      if (!kinoPlan(startState, endState, time_now, path_pts,
                    kinojerk_path_finder_)) {
  ...
  visualization_->displayKinoAStarList(path_pts, Eigen::Vector4d(0.8, 1, 0, 1),
                                       0);
  // step four: unconstrained optimziation
  if (!poly_traj_solver_.minJerkTrajOpt(inner_pts, allo_ts, startState,
                                        endState,
                                        time_now,  // ros::Time
                                        hPolys)) {
...
bool PlannerManager::kinoPlan(Eigen::MatrixXd &startState,
                              Eigen::MatrixXd &endState, ros::Time plan_time,
  ...
  //   Eigen::Vector3d end_point = kino_path.back() + 0.01 * ( kino_path.back()
  //   -  kino_path[path_size-1]); kino_path.push_back(end_point);
...
bool PlannerManager::getSikangConst(std::vector<Eigen::Vector3d> &path_pts,
  ...
  std::cout << "[PlannerManager]:map_vertical_bbxis "
            << map_vertical_bbx.transpose() << std::endl;

  decomp_util_.set_obs(map_util_->getCloud());
  decomp_util_.set_local_bbox(Vec3f(bb_xy_size_, bb_xy_size_, bb_z_up_),
                              Vec3f(bb_xy_size_, bb_xy_size_, bb_z_down_));
  ...
  Eigen::Vector3d end_point =
      path_pts.back() + 0.01 * (path_pts.back() - path_pts[path_size - 1]);
  ...
  for (size_t i = 0; i < path_size; i++) {
    ...
    if (i > 0) {
      if (opt_planner::insidehPoly(hPoly, path_pts[query_index]) &&
          cnt_num <= 20) {
        // std::cout << "the seed point is inside ! : " << path_pts[query_index]
        // << std::endl;
        cnt_num++;
        continue;
      } else {
        // std::cout << "the seed point is not inside ! : " <<
        // path_pts[query_index] << std::endl; seed point is not inside
        query_index = i - 1;
        cnt_num = 0;
      }
      ...
      // std::cout << "the query_index * time_res_  is " << query_index *
      // time_res_ << std::endl;
    }

    // get one polyhedron

    // std::cout << "the path_pts.at(query_index) is " <<
    // path_pts.at(query_index) << std::endl;
  ...
  // std::cout << "the temp_hPolys.size() is " << temp_hPolys.size() <<
  // std::endl;
  ...
  if (M > 8) {
    ...
    for (int i = M - 1; i >= 0; i--) {
      for (int j = 0; j < i; j++) {
        ...
        if (is_overlap) {
          if (j < i - 1 &&
              (!opt_planner::insidehPoly(temp_hPolys[i], temp_pts.at(j)))) {
            continue;
          }
          idices.push_front(j);
          i = j + 1;
    ...
    for (const auto &ele : idices) {
      ...
      if (j < short_cut_size) {
        ...
        // std::cout << "htemp_pts.at(ele) is " << temp_pts.at(ele) <<
        // std::endl;
      }
      last_ele = ele + 1;
      j += 1;
  ...
  // DecompROS::polyhedron_array_to_ros(poly_disp); poly_msg.header.frame_id =
  // frame_id_; poly_pub_.publish(poly_msg);
  if (this->if_write_poly_file_) {
    Eigen::IOFormat matformat(Eigen::StreamPrecision, 0, ", ", "\n", "", "", "",
                              "\n");
    ofstream poly_array_file;
    poly_array_file.open("/home/yifei/h_poly" +
                         std::to_string(this->local_planner_iteration_) +
                         ".txt");
    ...
    poly_array_file.open("/home/yifei/path_pts" +
                         std::to_string(this->local_planner_iteration_) +
                         ".txt");
    poly_array_file << inner_pts.format(matformat);
    poly_array_file.close();
    poly_array_file.open("/home/yifei/allo_ts" +
                         std::to_string(this->local_planner_iteration_) +
                         ".txt");
