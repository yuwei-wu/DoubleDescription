...
ros::Publisher pub_path, pub_gt_path;
ros::Subscriber sub_vicon_odom;
ros::Publisher pub_point_cloud, pub_margin_cloud;
ros::Publisher pub_key_poses;
ros::Publisher pub_camera_pose;
ros::Publisher pub_camera_pose_visual;
nav_msgs::Path path, vicon_path;
...
void vicon_callback(const nav_msgs::Odometry::ConstPtr &pose_msg)
{

    if(!set_start_pose){
      start_pos << pose_msg->pose.pose.position.x, pose_msg->pose.pose.position.y, pose_msg->pose.pose.position.z;
      set_start_pose = true;
    }
    ...
    pose_stamped.pose.position.x -= start_pos(0);
    pose_stamped.pose.position.y -= start_pos(1);
    pose_stamped.pose.position.z -= start_pos(2);

    vicon_path.header = pose_msg->header;
    vicon_path.header.frame_id = "map";
    vicon_path.poses.push_back(pose_stamped);
    pub_gt_path.publish(vicon_path);
...
void registerPub(ros::NodeHandle &n)
{
    ...
    //temp revision @yuwei
    sub_vicon_odom = n.subscribe("/vicon/f250_ariel/odom", 2000, vicon_callback);
    pub_gt_path = n.advertise<nav_msgs::Path>("vicon_path", 1000);
